...
export default function GestionIncidencias() {
  ...
  // Estado para modal de asignación de técnico (incidencias pendientes sin técnico)
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      // Solo cargar lista — calle se obtiene al abrir el panel (evita saturar Nominatim)
  ...
  // Al abrir panel lateral, cargar dirección
  const abrirPanel = async (inc: Incidencia) => {
    setIncidenciaSeleccionada(inc)
    setCallePanel('Calculando...')
    const calle = callesCache.current[inc.id] ?? await getCalle(parseFloat(inc.latitud), parseFloat(inc.longitud))
    callesCache.current[inc.id] = calle
    setCallePanel(calle)
  }

  // Abrir modal de asignación — carga técnicos con especialidad en la categoría
  ...
  // Confirmar asignación de técnico a incidencia pendiente
  const handleAsignarTecnico = async () => {
    if (!incidenciaAsignar || !tecnicoSeleccionado) return
    setProcesandoAsignacion(true)
    try {
      // Crear tarea urgente reutilizando el endpoint existente
      const res = await fetch('/api/admin/tareas', {
        ...
        body: JSON.stringify({
          ...
          descripcion: incidenciaAsignar.reportes[0]?.descripcion ?? 'Reasignación manual',
          tecnicoId: tecnicoSeleccionado,
          incidenciaId: incidenciaAsignar.id  // pasar incidencia existente para no crear duplicado
        })
      })
      const data = await res.json()
      if (!res.ok) { mostrarMensaje('error', data.error ?? 'Error al asignar'); return }
      mostrarMensaje('ok', '✅ Técnico asignado correctamente')
  ...
  const handleCancelarTarea = async () => {
    ...
    try {
      ...
      mostrarMensaje('ok', '✅ Tarea cancelada. La incidencia volvió a pendiente.')
  ...
  const filtradas = filtro === 'todos' ? incidencias : incidencias.filter(i => i.estado === filtro)

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Gestión de incidencias</h1>
      ...
      <div className="flex gap-2">
        {FILTROS.map(f => (
          ...
          >
            {FILTRO_LABELS[f]}
            {f !== 'todos' && <span className="ml-1.5 opacity-70">({incidencias.filter(i => i.estado === f).length})</span>}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-600 font-medium">Sin incidencias {filtro !== 'todos' ? FILTRO_LABELS[filtro].toLowerCase() : 'activas'}</p>
        </div>
      ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-2 flex justify-end gap-2" onClick={e => e.stopPropagation()}>
                    {/* Botón asignar técnico — solo para incidencias sin técnico */}
                    {sinAsignar && (
                      <button
                    ...
                    {tareaActiva && ['asignada','aceptada','en_curso','atrasada'].includes(tareaActiva.estado) && (
                      <button
                        onClick={() => {
                          setTareaACancelar({ id: tareaActiva.id, tecnico: tecnico?.nombre ?? 'técnico' })
      ...
      {incidenciaSeleccionada && !showModalCancelar && !showModalAsignar && (
        <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setIncidenciaSeleccionada(null)}>
          <div className="w-full max-w-md bg-white h-full shadow-2xl overflow-y-auto border-l border-slate-200" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
              ...
              <button onClick={() => setIncidenciaSeleccionada(null)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400">✕</button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              {incidenciaSeleccionada.reportes[0] && (
                <div className="bg-slate-50 rounded-xl p-4">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Descripción ciudadano</p>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-400">Días sin resolución</span>
                  <span className={`font-bold ${diasSinAtencion(incidenciaSeleccionada.creado_en) >= 7 ? 'text-red-600' : 'text-slate-700'}`}>
                    {diasSinAtencion(incidenciaSeleccionada.creado_en)} días
                  </span>
                </div>
                <div className="flex justify-between items-start gap-4">
                  <span className="text-slate-400 flex-shrink-0">Ubicación</span>
                  <span className="text-slate-700 text-xs text-right">📍 {callePanel}</span>
                </div>
              </div>

              {/* Botón asignar técnico si la incidencia está pendiente sin tarea */}
                    ...
                    <div key={tarea.id} className="border border-slate-200 rounded-2xl p-4 space-y-3">
                      ...
                      {tarea.motivo_atraso && (
                        <div className="bg-red-50 rounded-xl p-3">
                          <p className="text-xs text-red-600 font-semibold">⚠️ Motivo atraso: {tarea.motivo_atraso}</p>
                        </div>
                      )}
                      {['asignada','aceptada','en_curso','atrasada'].includes(tarea.estado) && (
                        <button
                          onClick={() => {
                            setTareaACancelar({ id: tarea.id, tecnico: tarea.tecnico?.nombre ?? 'técnico' })
                            setShowModalCancelar(true)
                          }}
      ...
      {/* ── Modal asignar técnico a incidencia pendiente ─────────────────── */}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setShowModalAsignar(false)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
              <div>
                <h3 className="text-base font-bold text-slate-900">Asignar técnico</h3>
                <p className="text-xs text-slate-400">Incidencia #{incidenciaAsignar.id} — {incidenciaAsignar.categoria.nombre}</p>
              </div>
              <button onClick={() => setShowModalAsignar(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400">✕</button>
            </div>

            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <p className="text-sm text-slate-500">
                Selecciona el técnico que atenderá esta incidencia. Se muestra su carga de trabajo actual.
              </p>
              ...
              {tecnicosDisponibles.length === 0 ? (
                <div className="text-center py-10 bg-slate-50 rounded-2xl">
                  ...
                  <p className="text-slate-400 text-sm mt-1">No existen técnicos con especialidad en {incidenciaAsignar.categoria.nombre}</p>
                </div>
              ) : (
                <div className="space-y-2">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setShowModalCancelar(false)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowModalCancelar(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400">✕</button>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
              <p className="text-sm text-amber-800">
                La tarea asignada a <strong>{tareaACancelar.tecnico}</strong> será cancelada y la incidencia volverá a estado <strong>pendiente</strong> para ser reasignada.
              </p>
            </div>
            <div>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setShowModalCancelar(false); setMotivoCancelacion('') }}
                className="flex-1 border-2 border-slate-200 rounded-2xl py-3 text-sm font-semibold text-slate-600 hover:bg-slate-50">
                Cancelar
              </button>
              <button onClick={handleCancelarTarea} disabled={!motivoCancelacion.trim() || procesando}
